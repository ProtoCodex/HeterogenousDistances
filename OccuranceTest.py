# -*- coding: utf-8 -*-
"""
Created on Wed Mar  8 21:57:38 2023
...
data = pd.read_csv("WFH_WFO_dataset_meth1.csv").drop(["ID", "Name"], axis=1)
y = data["Target"]
...
# mapper = ws.ordinalOccurance(data, y, [x for x in continuous_attributes if x != "Age"])
...
from sklearn.metrics.pairwise import euclidean_distances
from sklearn.metrics import accuracy_score, precision_score, recall_score, f1_score
print("################################################# Method 1 for WFH_WFO_dataset.csv #######################################################")
k = 3
rseed = [42, 53, 67, 80, 99]
le = LabelEncoder()
# Base HVDM for WFH_WFO_dataset.csv
data = pd.read_csv("pAttrition_meth1.csv")
...
# mapper = ws.ordinalOccurance(data,y, ordinal_attributes)
...
data = pd.read_csv("Employee_meth1.csv")
y = data["LeaveOrNot"]
